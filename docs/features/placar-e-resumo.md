...
Responder "como foi meu período" em números. O **cabeçalho** (`#placar`) é uma faixa horizontal no topo do painel: perfil, placar, aproveitamento, comparativo e o gráfico de rating. Os quatro mini-cards (`#resumo`) ficam na mesma faixa, entre o placar e o gráfico. Ver [layout](layout.md).

## Como usar

Tudo isso vive na faixa do topo, acima das duas colunas. Com mais de uma modalidade no período, abas (bullet/blitz/rápida/diária) trocam o recorte de tudo — placar, resumo, diagnóstico, evidência e lista.
...
- **Mini-cards** (`renderResumo`): forma recente, brancas × pretas, sequência atual e ritmo. Não há mais resumo em prosa: a frase repetia o que o tally e a `.placarInfo` dizem em números na mesma faixa.
