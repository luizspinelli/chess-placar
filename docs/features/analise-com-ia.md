...
- **Analisar lances (últimas 100)** — envia uma linha por partida com os 15 primeiros lances, marcos e relógio; devolve Leitura geral, O que manter, O que parar de fazer, O que estudar, Como acompanhar.
...
Com uma chave gratuita do Gemini: Analisar indicadores em `7d` (deve sair "PERÍODO CURTO", sem plano de 2 semanas) e em `ano` (com "O que mudou"). Analisar lances antes e depois de rodar o motor: a segunda resposta pode citar lances; a primeira não. Sem chave, a mensagem "Informe a chave da API". Exportar PDF com e sem análise.
