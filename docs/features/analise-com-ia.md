...
- A resposta passa por `mdParaHtml` (títulos, listas, negrito, itálico) com HTML escapado. Cada título vira uma `<section>`, e o CSS distribui as seções em **colunas** (`columns: 28rem`: 3 num monitor largo, 2 num notebook, 1 no celular) como cartões que não quebram no meio — a resposta ocupa o painel inteiro mantendo linhas legíveis, em vez de uma coluna de 78 caracteres com o resto da tela vazio.
