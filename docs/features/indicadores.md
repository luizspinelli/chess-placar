...
Os achados automáticos e o bloco da IA formam o **Diagnóstico**, que vive num modal de tela cheia — não é aba nem coluna. O botão da barra de contexto abre ele, já mostrando o sinal do achado mais grave e quantos são. Abaixo dela, a seção **Evidência**, com nove abas agrupadas em **Visão geral** (Resultados, Rating), **Como você joga** (Aberturas, Lances e relógio, Erros e precisão) e **Contexto** (Adversários, Sessões, Horários, Volume). Todo card com conceito não óbvio tem um "?" com a explicação (`GLOSSARIO`) — no mouse aparece ao passar; no toque, um balão (`#dica`, em `app.js`) mostra o texto ao tocar, e o mesmo vale para as células do mapa de calor e para os ▲/▼ da faixa comparativa. No celular a barra de abas vira um `<select>` com os três grupos como `optgroup`, fixo no topo ao rolar; as abas em quatro linhas comiam 200 px a cada troca. A Evidência é a coluna principal do painel, com a barra de abas fixa e o grid rolando por dentro (ver [layout](layout.md)). Os cards não formam um grid de trilhas, e sim **linhas de flexbox** (`flex-wrap`), cada card com `flex:1 1 360px` e o largo (`span2`) com `flex:2 1 740px`. O motivo é medido: com trilhas fixas, uma linha que mistura card largo com card normal deixa trilhas mortas, e a última linha incompleta sempre sobra — davam de 16% a 67% de vazio por aba, dependendo de quantos cards a aba tem. Com flex, os cards da linha crescem até fechar a largura, sempre. Medido nas nove abas com 760 partidas: cobertura média de 72% para **88%**, e **zero linhas furadas** (a aba Adversários saiu de 67% de sobra numa linha para nenhuma).

A caixa mede a **coluna**, não a janela: `#kpis` é um `container-type:inline-size` e as regras vêm de `@container`, porque recolher a coluna de partidas leva a evidência de 1349 px para 1884 px e uma media query da janela não enxergaria isso. `align-items:flex-start` mantém cada card na altura do próprio conteúdo, em vez de esticá-lo à do vizinho — aquilo era moldura vazia.

O que **não** se resolve com layout: quando uma aba tem pouco conteúdo, sobra altura na coluna. Encher isso esticando cards produziria cards altos e vazios, que é pior que espaço vazio honesto. A barra de abas **nunca rola**: quebra em linhas em qualquer largura — a rolagem horizontal com barra escondida deixava abas fora da tela tanto no celular quanto na coluna estreita do desktop.
...
Com um nick de muito volume (`hikaru`, `ano`), abrir o Diagnóstico e **clicar** em Analisar, Exportar PDF, configurar, ↻ e Só o motor — `tests/delegacao.test.js` cobre a ligação dos listeners, mas só o clique real prova o fluxo. Depois, passar pelas nove abas: nenhum card vazio sem o texto "Sem dados"; barras de aproveitamento somando com o placar; achados com números que batem com os cards. Com `7d` de um jogador casual, conferir que a maioria dos achados **não** dispara.
