...
No modo avançado, dez abas agrupadas em **Visão geral** (Análise, Resultados, Rating), **Como você joga** (Aberturas, Lances e relógio, Precisão) e **Contexto** (Adversários, Sessões, Horários, Volume). Todo card com conceito não óbvio tem um "?" com a explicação (`GLOSSARIO`). No celular as abas quebram em linhas em vez de rolar.
