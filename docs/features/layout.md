...
- A coluna de **Partidas recolhe** (`body.listaAberta`): fechada, ela sai da faixa e vira uma linha no rodapé, e a grade passa a duas colunas — a Evidência vai de 651 px para ~988 px numa tela de 1366 px, trocando duas colunas de cards por três. O bootstrap liga `listaAberta` acima de 1100 px de largura; no celular ela começa recolhida, como sempre.
- O **Diagnóstico abre em tela cheia** pelo "⤢" do seu cabeçalho (`analiseFullscreen`). A seção inteira é **movida** para dentro de `#modalAnalise` e devolvida ao `#colDiag` ao fechar — nunca clonada: o campo da chave da IA vive dentro dela, e clonar duplicaria ids e perderia o que estivesse digitado. Fecha no ✕, no clique no fundo e no Esc.
...
- **Tela cheia em vez de coluna maior para o diagnóstico.** Na coluna ele tem ~485 px; em tela cheia, ~1870 px, e os achados voltam a três colunas. Alargar a coluna permanentemente roubaria da Evidência, que é quem precisa de largura o tempo todo.
