...
- Depois de mexer em código compartilhado, percorrer com o console aberto: tela inicial → busca → barra de contexto (rótulo, painel abrindo e fechando pelos três caminhos) → as dez abas de KPI → modal do gráfico → modal da abertura → modo simples → lista de partidas → largura de celular. Usar um nick real (`?nick=hikaru`) ou o botão "Veja um exemplo".
