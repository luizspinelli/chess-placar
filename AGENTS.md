...
- Antes de mexer num domínio, ler a doc dele — tem os limiares e o porquê das decisões, que o código não diz:
  ...
  - `js/placar.js`, `js/partidas.js`, `js/grafico.js`, `js/overlay.js`, `js/app.js` → `placar-e-resumo.md`, `lista-de-partidas.md`, `grafico-de-rating.md`, `modo-streamer.md`, `parametros-de-url.md`
...
- Depois de mexer em código compartilhado, percorrer com o console aberto: tela inicial → busca → as dez abas de KPI → modal do gráfico → modal da abertura → modo simples → streamer (`?streamer=1&overlay=ticker`) → largura de celular. Usar um nick real (`?nick=hikaru`) ou o botão "Veja um exemplo".
